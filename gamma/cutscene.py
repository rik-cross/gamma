...
class Cutscene:
    ...
    def draw(self, scene):
        ...
        Rectangle(
            0, 0,
            windowSize.w, ceil(self.lettboxHeight * (self.fadePercentage / 100)),
            colour=self.lettboxColour
        ).draw(scene.surface)

        #bottom letterbox
        Rectangle(
            0, ceil(windowSize.h - (self.lettboxHeight * (self.fadePercentage / 100))),
            windowSize.w, self.lettboxHeight * (self.fadePercentage / 100),
            colour=self.lettboxColour
        ).draw(scene.surface)
